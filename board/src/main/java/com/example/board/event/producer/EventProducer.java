package com.example.board.event.producer;
        ...
        kafkaTemplate.send(topic, outbox.getPayload()) // TODO: 파티션 늘리고 key 명시 - kafkaTemplate.send(topic, key, outbox.getPayload())
                .whenComplete((result, ex) -> {
                    if (ex != null) {
                        ...
                        log.error("[board-service] 메시지 에러 발생 :: ID: {}, Error: {}", outbox.getId(), ex.getMessage());
                    } else {
                        // 아웃박스에 발송 성공 업데이트
                        outbox.markSent();
                        outboxEventRepository.save(outbox);
                        log.info("[board-service] 메시지 성공 :: {}", result.getRecordMetadata());
                    }
                });
    }
}
